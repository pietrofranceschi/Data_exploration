...
```
    ...
    ## Warning in data("data/iris"): data set 'data/iris' not found
...
RStudio or issue `ls` on the console

``` r
ls()
```

    ## [1] "counts"   "countsn"  "gasoline" "vintages" "wines"
...
First of all let’s look to the number of variables and the number of
samples
...
table for the factor. What can we earn from these numbers?
  ...
  - **Normality**: If the mean and the median are almost equal, this is
    ...
    for normality however. We will see `qqnorm` for that
