...
-   **Normality**: If the mean and the median are almost equal, this is a good indication that the variable under consideration has a symmetric distribution around the mean. This is not a very good test for normality however. We will see `qqnorm` for that
...
Some of these facts are not unexpected since our dataset is made up of three different iris species ... To see this we should include the specie information in the previous plots. There are fast ways of doing this by using `ggplot2`. Here we want to use only simple commands so we look to Petal.Length in the three species
